...
#define MAXINODE 50     // Total Number of Inodes means you can create Maximum 50 Files

#define READ 1          // You are Opening this File to Read 
#define WRITE 2         // You are Opening this File for Write

#define MAXFILESIZE 2048    // Size of one file is MAx 2048

#define REGULAR 1       //  We can create Regular file but project has both files support
#define SPECIAL 2
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    if((UFDTArr[fd].ptrfiletable->ptrinode->FileType) != REGULAR)  return -33;
...
int stat_file(char *name)
{
    ...
    printf("\n------------Sattistical Information about File-------------\n");
    printf("File Name : %s\n", temp->FileName);
    printf("Inode Number : %d\n",temp->InodeNumber);
    printf("File Size : %d\n", temp->FileSize);
    printf("Actual File Size : %d\n",temp->FileActualSize);
...
int main()
{
    ...
    InitialiseSuperBlock();             // Auxilary Data Initialisation
    CreateDILB();                       // Auxilary Data Initialisation

    while (1)                           // it is known as Shell & Shell is Infinite listening Loop
    {
        fflush(stdin);                  // To Clear the Buffer
        strcpy(str,"");                 // To Clear str String

        printf("\nSanket's VFS : > ");

        fgets(str,80,stdin);            // create A.txt 3  || 80 - Max Terminal Letter
        //scanf("%[^'\n']s,str");
        ...
        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);
            ...
            if(count == 1)
            {
                ...
                {
                    printf("Terminating from the Sanket's Virtual File System\n");
                    break;
                }
                else
                {
            ...
            else if(count == 3)
            {
                ...
                else if(strcmp(command[0],"read") == 0)
                {
                    ...
                    if(ret == -2)
                        printf("ERROR : Permission Denied\n");
                    if(ret == -3)
                        printf("ERROR : Reached at the End od File\n");
                    if(ret == -4)
                        printf("ERROR : It is Not Regular File\n");
                    if(ret == 0)
